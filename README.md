...
where $X R^{p \times J$ is the data matrix and $\bar{x}$ is the sample mean vector. $W$ is the spatial weight matrix, where the (k, h) element of matrix, $w_{k,h}$ quantifies the spatial dependency between the k-th and h-th pixels.
